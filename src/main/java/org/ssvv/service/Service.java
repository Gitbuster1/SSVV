...
import org.ssvv.domain.*;
import org.ssvv.repository.*;
...
public class Service {
    ...
    public int saveStudent(String id, String nume, String grupa) {
        int groupInt = Integer.parseInt(grupa);
        int idInt = Integer.parseInt(id);
        if(groupInt<=0 || idInt <=0)
            return 0;
        if(nume==null)
            return 0;
        Student student = new Student(id, nume, groupInt);
